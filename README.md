...
**Docker + HTTPS (production):** Default **`docker-compose.yml`** uses **`nginx/nginx-no-ssl.conf`**, port **80** only, and mounts **`/var/www/certbot`** so **certbot** can complete the ACME challenge. Install certbot on the host, run `sudo mkdir -p /var/www/certbot`, bring the stack up, then e.g. `sudo certbot certonly --webroot -w /var/www/certbot -d microtechnique.in -d www.microtechnique.in`. After **`/etc/letsencrypt/live/microtechnique.in/`** exists, switch nginx to TLS: either **replace** the `nginx` service in `docker-compose.yml` with the one in **`docker-compose.tls.yml`**, or run `docker compose -f docker-compose.yml -f docker-compose.tls.yml up -d --force-recreate nginx` (if merge issues, copy-paste the TLS `nginx` block manually). Set **`CORS_ORIGIN`** and **`PUBLIC_APP_URL`** to **`https://…`** in `.env` once you serve HTTPS. **Do not** use `nginx/nginx.conf` in Compose until the cert files exist — nginx will exit and nothing will listen on port 80.
